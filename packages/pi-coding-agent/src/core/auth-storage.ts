...
export class AuthStorage {
	...
	/**
	 * Escalation counter per credential for repeated rate-limit backoffs.
	 * Key: "provider:credentialIndex", value: consecutive-hit count.
	 * Used to exponentially increase backoff duration when the same credential
	 * keeps getting rate-limited.
	 */
	private credentialEscalation: Map<string, number> = new Map();
	...
	private normalizeProviderEntry(
		...
		let rawCredentials: AuthCredential[];
		let changed = false;
		if (Array.isArray(entry)) {
			rawCredentials = entry;
		} else if (
			// Handle objects with numeric string keys ("0", "1", "2", ...)
			// that were stored as an object instead of a JSON array.
			typeof entry === "object" &&
			!("type" in entry) &&
			Object.keys(entry).some((k) => /^\d+$/.test(k))
		) {
			const numericKeys = Object.keys(entry)
				.filter((k) => /^\d+$/.test(k))
				.sort((a, b) => Number(a) - Number(b));
			rawCredentials = numericKeys.map((k) => (entry as Record<string, AuthCredential>)[k]);
			changed = true; // will be re-saved as a proper array
		} else {
			rawCredentials = [entry];
		}
	...
	async refreshProviderRateLimitInfo(provider: string, sessionId?: string): Promise<CredentialRateLimitInfo[]> {
		...
		credentials.forEach((credential, index) => {
			...
			// Only clear an existing backoff if the probe succeeded (no error) and
			// confirmed the credential is NOT rate-limited.  For API-key credentials
			// the probe always fails with an error — clearing the backoff here would
			// defeat the client-side 30 s safety window applied by markUsageLimitReached.
			if (!snapshot.isRateLimited && !snapshot.error) {
				providerBackoff.delete(index);
			}
		});
	...
	markUsageLimitReached(
		provider: string,
		sessionId?: string,
		options?: { errorType?: UsageLimitErrorType; retryAfterMs?: number },
	): boolean {
	...
	markUsageLimitReachedWithFallback(
		...
		// Transport/unknown failures on a single credential should fall through to
		// normal exponential backoff. Real usage-limit errors must still back off
		// the only credential so cross-provider fallback and recovery waiting can
		// engage correctly.
		if (credentials.length === 1 && errorType === "unknown") {
			return { hasAlternate: false, usedCredential: credentials[0] };
		}

		// Use server-reported retry delay when provided; fall back to default durations
		const backoffMs = options?.retryAfterMs ?? getBackoffDuration(errorType);
		...
		// Never shorten an existing, longer backoff (e.g. from a server-reported
		// reset time).  Also escalate when the same credential is hit repeatedly:
		//   30s → 60s → 2min → 5min → 10min (capped).
		let providerBackoff = this.credentialBackoff.get(provider);
		if (!providerBackoff) {
			providerBackoff = new Map();
			this.credentialBackoff.set(provider, providerBackoff);
		...
		for (const index of indexesToBackOff) {
			const existing = providerBackoff.get(index);
			const existingRemaining = existing ? Math.max(0, existing - Date.now()) : 0;

			// Track consecutive backoffs per credential for escalation
			const escalationKey = `${provider}:${index}`;
			if (existingRemaining > 0) {
				// Credential was ALREADY backed off — escalate
				this.credentialEscalation.set(escalationKey, (this.credentialEscalation.get(escalationKey) ?? 0) + 1);
			} else {
				// Fresh backoff — reset escalation
				this.credentialEscalation.set(escalationKey, 0);
			}

			const escalation = this.credentialEscalation.get(escalationKey) ?? 0;
			const escalatedMs = Math.min(backoffMs * 2 ** escalation, 10 * 60_000); // cap 10min
			const newBackoffUntil = Date.now() + escalatedMs;

			// Never shorten an existing backoff
			if (!existing || newBackoffUntil > existing) {
				providerBackoff.set(index, newBackoffUntil);
			}
		}
